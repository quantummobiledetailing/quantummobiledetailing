  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(packages, (servicePackage, index) => {
        ...
        if (!servicePackage.hourly) {
          _push(`<p class="text-[10px] uppercase tracking-[0.3em] text-slate-500">Starts at</p>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<p class="text-2xl font-semibold text-white">${ssrInterpolate(servicePackage.price)}`);
        if (!servicePackage.hourly) {
          _push(`<span>+</span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</p></div></div><p class="text-xs uppercase tracking-[0.3em] text-slate-400">Time: ${ssrInterpolate(servicePackage.duration)}</p><h2 class="mt-4 text-2xl font-semibold text-white">${ssrInterpolate(servicePackage.title)}</h2><p class="mt-2 text-sm text-slate-300">${ssrInterpolate(servicePackage.summary)}</p><ul class="mt-4 space-y-2 text-sm text-slate-200"><!--[-->`);
...
//# sourceMappingURL=services-sM1WDbMZ.js.map
