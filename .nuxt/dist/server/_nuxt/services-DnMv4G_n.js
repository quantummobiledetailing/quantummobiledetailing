import { defineComponent, ref, mergeProps, createVNode, resolveDynamicComponent, unref, useSSRContext } from "vue";
import { ssrRenderAttrs, ssrRenderList, ssrRenderAttr, ssrInterpolate, ssrRenderVNode, ssrRenderTeleport } from "vue/server-renderer";
import { C as ClipboardDocumentListIcon, T as TruckIcon, S as SparklesIcon } from "./TruckIcon-CpaYUs5V.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></div><div class="grid gap-6 lg:grid-cols-3"><article class="glow-card card-hover flex h-full flex-col rounded-3xl p-6 reveal" data-reveal><div class="flex items-start justify-between gap-4"><span class="text-xs uppercase tracking-[0.3em] text-brand-primary/70">${ssrInterpolate(ceramicPackage.tier)}</span><div class="text-right"><p class="text-[10px] uppercase tracking-[0.3em] text-slate-500">Starts at</p><p class="text-2xl font-semibold text-white">${ssrInterpolate(ceramicPackage.price)}+</p></div></div><p class="text-xs uppercase tracking-[0.3em] text-slate-400">Time: ${ssrInterpolate(ceramicPackage.duration)}</p><h2 class="mt-4 text-2xl font-semibold text-white">${ssrInterpolate(ceramicPackage.title)}</h2><p class="mt-2 text-sm text-slate-300">${ssrInterpolate(ceramicPackage.summary)}</p><ul class="mt-4 space-y-2 text-sm text-slate-200"><!--[-->`);
      ssrRenderList(ceramicPackage.features, (item) => {
        _push(`<li>- ${ssrInterpolate(item)}</li>`);
      });
      _push(`<!--]--></ul><button type="button" class="mt-6 btn-outline text-center"> Call About ${ssrInterpolate(ceramicPackage.title)}</button></article><article class="glow-card card-hover flex h-full flex-col rounded-3xl p-6 lg:col-span-2 reveal" data-reveal data-reveal-delay="120"><p class="text-xs uppercase tracking-[0.4em] text-brand-primary/80">Add-ons</p><div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><!--[-->`);
      ssrRenderList(addOns, (addon, index) => {
        _push(`<div class="rounded-2xl border border-white/10 p-4 reveal" data-reveal${ssrRenderAttr("data-reveal-delay", index * 50)}><h3 class="text-lg font-semibold text-white">${ssrInterpolate(addon.title)}</h3><p class="text-sm text-slate-300">${ssrInterpolate(addon.description)}</p><p class="mt-3 text-sm font-semibold text-brand-primary">${ssrInterpolate(addon.price)}</p></div>`);
      });
      _push(`<!--]--></div></article></div></section><section class="mx-auto max-w-6xl reveal" data-reveal><article class="glow-card card-hover rounded-3xl p-6 reveal" data-reveal><p class="text-xs uppercase tracking-[0.4em] text-brand-primary/80">Process Breakdown</p><div class="mt-6 grid gap-6 md:grid-cols-3"><!--[-->`);
      ssrRenderList(detailSections, (section, index) => {
        _push(`<div class="rounded-2xl border border-white/10 p-4 reveal" data-reveal${ssrRenderAttr("data-reveal-delay", index * 80)}><div class="flex items-center gap-3">`);
...
//# sourceMappingURL=services-DnMv4G_n.js.map
