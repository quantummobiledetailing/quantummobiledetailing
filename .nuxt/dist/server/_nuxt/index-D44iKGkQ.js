...
import { defineComponent, ref, mergeProps, withCtx, createTextVNode, createVNode, resolveDynamicComponent, unref, useSSRContext } from "vue";
import { ssrRenderAttrs, ssrRenderAttr, ssrRenderComponent, ssrRenderList, ssrInterpolate, ssrRenderVNode, ssrRenderTeleport } from "vue/server-renderer";
import { publicAssetsURL } from "#internal/nuxt/paths";
import { T as TruckIcon, C as ClipboardDocumentListIcon, S as SparklesIcon } from "./TruckIcon-CpaYUs5V.js";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div></div><div class="hero-stats-grid"><!--[-->`);
      ssrRenderList(heroStats, (stat, index) => {
        _push(`<div class="glow-card card-hover rounded-3xl px-6 py-6 text-left reveal" data-reveal${ssrRenderAttr("data-reveal-delay", index * 80)}><p class="text-xs uppercase tracking-[0.4em] text-brand-primary/70">${ssrInterpolate(stat.label)}</p><p class="mt-3 text-3xl font-semibold text-white sm:text-4xl">${ssrInterpolate(stat.value)}</p></div>`);
      });
      _push(`<!--]--></div></div></div></section><section class="px-6 sm:px-10 reveal" data-reveal><div class="mx-auto grid max-w-6xl gap-6 md:grid-cols-3"><!--[-->`);
      ssrRenderList(features, (feature, index) => {
        _push(`<article class="glow-card card-hover rounded-3xl p-6 reveal" data-reveal${ssrRenderAttr("data-reveal-delay", index * 80)}><div class="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10">`);
      ...
      _push(`<!--]--></div></section><section class="bg-slate-900/40 px-6 py-16 sm:px-10 reveal" data-reveal><div class="mx-auto max-w-6xl space-y-10"><header class="space-y-4 text-center"><div class="flex items-center justify-center gap-2 text-brand-primary/80">`);
      _push(ssrRenderComponent(SparklesIcon, {
        class: "accent-spark h-4 w-4",
        "aria-hidden": "true"
      }, null, _parent));
      _push(`<p class="text-xs uppercase tracking-[0.4em] text-brand-primary/80">Signature Packages</p></div><h2 class="text-3xl font-semibold text-white">Hand-crafted plans to fit every finish</h2><p class="text-slate-300">Each package is tailored during your consultation, balancing your driving habits, vehicle type, and aftercare goals.</p></header><div class="grid gap-6 md:grid-cols-3"><!--[-->`);
      ssrRenderList(services, (service, index) => {
        _push(`<article class="glow-card card-hover flex flex-col rounded-3xl p-6 reveal" data-reveal${ssrRenderAttr("data-reveal-delay", index * 80)}><div class="flex items-center justify-between"><span class="text-xs uppercase tracking-[0.3em] text-brand-primary/70">${ssrInterpolate(service.tier)}</span><div class="text-right"><p class="text-[10px] uppercase tracking-[0.3em] text-slate-500">Starts at</p><p class="text-lg font-semibold text-white">${ssrInterpolate(service.price)}+</p></div></div><h3 class="mt-4 text-2xl font-semibold text-white">${ssrInterpolate(service.title)}</h3><p class="mt-2 text-sm text-slate-300">${ssrInterpolate(service.description)}</p><ul class="mt-4 space-y-2 text-sm text-slate-200"><!--[-->`);
        ssrRenderList(service.perks, (perk) => {
          _push(`<li>- ${ssrInterpolate(perk)}</li>`);
        });
        _push(`<!--]--></ul><button type="button" class="mt-6 inline-flex justify-center rounded-full border border-brand-primary px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-brand-accent hover:bg-brand-primary hover:text-black"> Call To Book </button></article>`);
      });
      _push(`<!--]--></div></div></section><section class="px-6 sm:px-10 reveal" data-reveal><div class="mx-auto grid max-w-6xl gap-6 md:grid-cols-2"><article class="glow-card card-hover rounded-3xl p-6 reveal" data-reveal><p class="text-xs uppercase tracking-[0.4em] text-brand-primary/70">Testimonials</p><h3 class="mt-3 text-2xl font-semibold text-white">Clients trust us with their daily drivers and collectibles.</h3><div class="mt-6 max-h-72 space-y-6 overflow-y-auto pr-2"><!--[-->`);
      ...
      _push(`<!--]--></div></article><article class="glow-card card-hover rounded-3xl p-6 reveal" data-reveal data-reveal-delay="120"><p class="text-xs uppercase tracking-[0.4em] text-brand-primary/70">Coverage Area</p><h3 class="mt-3 text-2xl font-semibold text-white">Proudly serving Saint Helens, Scappoose, Rainier, and the greater Oregon Northwest.</h3><p class="mt-3 text-sm text-slate-300"> We bring the tools, set up canopies, and tap into your available water and electrical outlets so detailing stays on schedule through rain, pollen, and chilly mornings. </p><p class="mt-3 text-sm text-slate-300"> Columbia County routes keep travel fees minimal, but we routinely head into Portland, Vancouver, and coastal towns when we negotiate fair trip pricing during your estimate. </p><p class="mt-3 text-sm text-slate-300"> For larger projects and exotic builds, we are happy to travel farther so we can care for supercars and hypercars the right way. Let us know the vehicle and location, and we will make a custom plan that fits the scope. </p></article></div></section>`);
...
//# sourceMappingURL=index-D44iKGkQ.js.map
